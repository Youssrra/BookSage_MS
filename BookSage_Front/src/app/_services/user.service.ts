...
export class UserService {
  ...
  public login(login:Login):Observable<any>{
      return this.httpclient.post("http://localhost:8888/users/signinn",login,{headers:this.requestHeader})
  }
  ...
  public logout(username:string):Observable<any>{
    ...
    return this.httpclient.post("http://localhost:8888/users/logout",body,{headers:this.requestHeader})
}
/*
  ...
  public test():Observable<any>{
    return this.httpclient.get("http://localhost:8888/test",{responseType:"text"});
  }

  public listeAdmin():Observable<any>{
    return this.httpclient.get("http://localhost:8888/users/getAllAdmin",{responseType:"json"});
  }

  public addAdmin(user:User):Observable<any>{
      return this.httpclient.post("http://localhost:8888/users/signup",user,{responseType:"text"})
  
  }

  public addc(user:User):Observable<any>{
    return this.httpclient.post("http://localhost:8888/users/AddClient",user,{responseType:"text"})

}


  public listeallClient():Observable<any>{
    return this.httpclient.get("http://localhost:8888/users/getAllClient",{responseType:"json"});
  }
  ...
  public resetpwd(email:string):Observable<any>{
    ...
    return this.httpclient.put(`http://localhost:8888/users/resetPwd/${email}`,{responseType:"text"});
  }
  public newpwd(code:string,pwd:string):Observable<any>{
    return this.httpclient.get(`http://localhost:8888/users/verifiePwd/${code}/${pwd}`);
  }
  ...
  public DetailsUser(userId:string):Observable<any>{
   // console.log(this.httpclient.get(`ttp://localhost:1112/users/${id}`,{responseType:"json"}))
      return this.httpclient.get(`http://localhost:8888/users/getUserById/${userId}`,{responseType:"json"});
  }

  
  public UpdateUserData(userId: string, user: any): Observable<any> {
    return this.httpclient.put(`http://localhost:8888/users/update/${userId}`, user, { responseType: "text" });
  }
  ...
  deleteClient(id: string, role: any): Observable<any> {
    ...
    if (role == 'ADMIN') {
      ...
      return this.httpclient.delete(`http://localhost:8888/users/remove/${id}`,{ headers }).pipe(
        tap((response: any) => {
